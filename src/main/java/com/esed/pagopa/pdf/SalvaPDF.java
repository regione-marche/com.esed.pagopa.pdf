...
//		creazione del pdf canvas
		...
		Text avvisoPagamentoText = new Text("AVVISO DI PAGAMENTO").setFont(asset.getTitoloFont());
		Paragraph avvisoPagamentoP = new Paragraph().add(avvisoPagamentoText).setFontColor(ColorConstants.BLACK)
				.setFontSize(asset.getInEvidenza1Size()).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);//FIXME uso inEvidenza1Size perché corrisponde alla combinazione di dimensioni del titolo delle pagine successive
		avvisoPagamentoCanvas.add(avvisoPagamentoP);
		avvisoPagamentoCanvas.close();
		...
		Paragraph oggettoPagamentoP = new Paragraph().add(oggettoPagamentoText).setFontColor(ColorConstants.BLACK)
				.setFontSize(asset.getImporto1Size()).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE).setFixedLeading(18); //FIXME uso getImporto1Size perché corrisponde alla combinazione di dimensioni del titolo delle pagine successive
		oggettoPagamentoCanvas.add(oggettoPagamentoP);
		oggettoPagamentoCanvas.close();
		...
		Text n5RataText4 = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 2).ScadenzaRata).setFont(asset.getInEvidenza5Font()).setFontSize(asset.getInEvidenza1Size()); //non è un errore in evidenza 3 e 1, serve una combinaizone diversa

...
public class InformazioniStampaBolzano implements InformazioniStampaInterface {
	...
	public String buildDate(String data) {
		...
		return new StringBuilder().append(data.subSequence(0, 3)).append(data.subSequence(4, 5))
				.append(data.subSequence(5, 6)).append("00:00:00Z").toString();
	}
	...
	private void gestioneCausale(String causale,PosizioneDebitoriaAvvisaturaDto posDeb) {
		String primaparte[] = new String[2];
		
		if(causale.contains("\n")) {
			primaparte = causale.split("\\n");
		}
		...
		if(prima.length==1) {
			...
			if(primaparte.length==2) {
			 posDeb.setCausaleDebitoria(primaparte[1].replaceAll("(\\r|\\n)", ""));
				...
				posDeb.setCausaleDebitoria(causale.replaceAll("(\\r|\\n)", ""));
			}
		} else {
			...
			if(prima!=null) {
			  ...
			  if(primaparte[1]==null) {
				posDeb.setCausaleDebitoria(causale);
			  }else {
				  posDeb.setCausaleDebitoria(primaparte[1].replaceAll("(\\r|\\n)", ""));
			  }
		}
	...
	private void gestioneDataMatrix(Documento doc,boolean daArchivio,
		PosizioneDebitoriaAvvisaturaDto posDeb) {
    ...
    private PosizioneDebitoriaAvvisaturaDto CreaPosizioneDebitoria(Documento doc,boolean daArchivio)
		{
		    PosizioneDebitoriaAvvisaturaDto posDeb  = new PosizioneDebitoriaAvvisaturaDto();

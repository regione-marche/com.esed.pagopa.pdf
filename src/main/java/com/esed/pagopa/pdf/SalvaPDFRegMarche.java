...
public class SalvaPDFRegMarche {
	...
	public SalvaPDFRegMarche(PropertiesTree propertiesTree) {
		
		this.propertiesTree = propertiesTree;
	}
	
	
	public static byte[] SalvaFile(Flusso flusso) throws IOException, ValidazioneException {
//		int stato = 1;
		ByteArrayOutputStream baos = null;
//		 TODO INVOCARE FUNZIONE METTIVIRGOLA PER GLI IMPORTI DI TUTTI I BOLLETTINI 

//		il flusso pu� contenere pi� di un documento
...
//			se il numero di bollettini � zero la stampa non parte
			int[] elencoBollettini = ValidaFlusso.validaFlussoRegMarche(flusso.Documentdata.get(i), flusso.TipoStampa);
//			chiude il metodo con stato a 1 se il numero dei bollettini � 0
...
//				se i bollettini sono 2 allora non c'è rateizzazione perché è il numero 1 e il 999 entrambi con dati coincidenti
...
//			Crea il file della guida e del pdf massivo, i file sono omonimi, cambia solo l'estensione
			...
			// creato il documento crea il lettore di asset
			LeggoAsset asset = new LeggoAsset(pdf, file512.tipoTemplate, flusso.Documentdata.get(0).DatiCreditore.get(0).LogoEnte);
...
//			});
	...
	private static int stampaDocumento(
			Flusso flusso, 
			LeggoAsset asset, 
			PdfDocument pdf, 
...
//		creazione del pdf canvas
		...
		// Avviso di Pagamento
		Rectangle avvisoPagamentoRectangle = new Rectangle(70, 800, 104, 15);
		Canvas avvisoPagamentoCanvas = new Canvas(pdfCanvas, avvisoPagamentoRectangle);
		Text avvisoPagamentoText = new Text("AVVISO DI PAGAMENTO").setFont(asset.getTitillium_bold());
		...
		// Logo Ente
		Rectangle logoEnteRectangle = new Rectangle(480, 732, 84, 84);
		Canvas logoEnteCanvas = new Canvas(pdfCanvas, logoEnteRectangle);
		logoEnteCanvas.add(asset.getLogo_ente().scaleToFit(84, 84));
		...
		Text oggettoPagamentoText = new Text(documento.CausaleDocumento).setFont(asset.getTitillium_bold());
		...
		// Ente Creditore
		Rectangle enteCreditoreRectangle = new Rectangle(32, 706, 89, 15);
		...
		Rectangle codiceFiscaleRectangle = new Rectangle(119, 707, 40, 12);
		Canvas codiceFiscaleCanvas = new Canvas(pdfCanvas, codiceFiscaleRectangle);
		Text codiceFiscaleText = new Text("Cod. Fiscale").setFont(asset.getTitillium_regular());
		...
		// CF Ente
		Rectangle cfEnteRectangle = new Rectangle(167, 708, 124, 10);
		...
		Paragraph cfEnteP = new Paragraph().add(cfEnteText).setFontColor(ColorConstants.BLACK).setFontSize(8)
		...
		// Destinatario Avviso
		Rectangle destinatarioAvvisoRectangle = new Rectangle(318, 706, 105, 15);
		...
		//		.setFont(asset.getTitillium_bold());
		Text destinatarioAvvisoText = new Text("DESTINATARIO AVVISO").setFont(asset.getTitillium_bold());
		//fine LP PG200420 - Errori nel pdf dell'avviso
		...
		// Codice Fiscale
		Rectangle codiceFiscaleRectangle2 = new Rectangle(435, 707, 40, 12);
		...
		// CF Destinatario
		Rectangle cfDestinatarioRectangle = new Rectangle(482, 708, 112, 10);
		...
		Rectangle enteCreditoreStringRectangle = new Rectangle(30, 670 - 17 + 4, 261 - 24 - 12, 48 - 8);
		if(bDebug)
		...
		Paragraph enteCreditoreStringP = new Paragraph().add(enteCreditoreStringText).setFontColor(ColorConstants.BLACK).setFontSize(12)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Rectangle nomeCognomeDestinatarioRectangle = new Rectangle(318, 670 - 17 + 4, 174 + 15, 48 - 8);
		if(bDebug)
		...
		//Nota. per retro comnpatibilità di usi diversi di pagonet web.
		//inizio LP PG21X007
		...
		Rectangle quantoQuandoPagareRectangle = new Rectangle(32, 568, 133, 15);
		Canvas quantoQuandoPagareCanvas = new Canvas(pdfCanvas, quantoQuandoPagareRectangle);
		Text quantoQuandoPagareText = new Text("QUANTO E QUANDO PAGARE?").setFont(asset.getTitillium_bold());
		...
		Rectangle pagamentoRatealeRectangle = new Rectangle(175, 569, 186, 12);
		...
		// Dove Pagare
		Rectangle dovePagareRectangle = new Rectangle(317, 568, 70, 15);
		Canvas dovePagareCanvas = new Canvas(pdfCanvas, dovePagareRectangle);
		Text dovePagareText = new Text("DOVE PAGARE?").setFont(asset.getTitillium_bold());
		Paragraph dovePagareP = new Paragraph().add(dovePagareText).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		dovePagareCanvas.add(dovePagareP);
		dovePagareCanvas.close();

		// Lista Canali Pagamento
		Rectangle listaCanaliPagamentoRectangle = new Rectangle(388, 569, 186, 12);
		...
		Text importoText = null;
		if(bollettino999.ScadenzaRata.contains("/")) {
			// Importo
			Rectangle importoRectangle = new Rectangle(31, 523, 99, 23);
			Canvas importoCanvas = new Canvas(pdfCanvas, importoRectangle);
			importoText = new Text(mettiVirgolaEPuntiAllImportoInCent(bollettino999.Codeline2Boll))
					.setFont(asset.getTitillium_bold());
			...
			importoCanvas.close();
			// Euro Data
			Rectangle euroDataRectangle = new Rectangle(110, 523, 178, 23);
			...
			Text euroDataText2 = new Text(" entro il ").setFont(asset.getTitillium_bold())
					.setFontSize(8);
			Text euroDataText3 = new Text(bollettino999.ScadenzaRata).setFont(asset.getTitillium_bold()).setFontSize(15);
			...
			// Importo
			Rectangle importoRectangle = new Rectangle(31, 521, 99, 23);
			Canvas importoCanvas = new Canvas(pdfCanvas, importoRectangle);
			importoText = new Text(mettiVirgolaEPuntiAllImportoInCent(bollettino999.Codeline2Boll))
					.setFont(asset.getTitillium_bold());
			Paragraph importoP = new Paragraph().add(importoText).setFontColor(ColorConstants.BLACK).setFontSize(15)
			...
			importoCanvas.close();
			// Euro Data
			Rectangle euroDataRectangle = new Rectangle(110, 521, 178, 23);
			Canvas euroDataCanvas = new Canvas(pdfCanvas, euroDataRectangle);
			Text euroDataText1 = new Text("Euro ").setFont(asset.getTitillium_bold()).setFontSize(15);
			Text euroDataText2 = new Text(" entro ").setFont(asset.getTitillium_bold())
					.setFontSize(8);
			Paragraph euroDataP = new Paragraph().add(euroDataText1).add(euroDataText2)
					.setFontColor(ColorConstants.BLACK).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
			euroDataCanvas.add(euroDataP);
			euroDataCanvas.close();
			
			//Dicitura scadenza
			Rectangle scadenzaRectangle = new Rectangle(30, 472, 243, 50);
			Canvas scadenzaCanvas = new Canvas(pdfCanvas, scadenzaRectangle);
			Text euroDataText3 = new Text(bollettino999.ScadenzaRata).setFont(asset.getTitillium_bold()).setFontSize(11);
			Paragraph scadenzaP = new Paragraph().add(euroDataText3)
					.setFontColor(ColorConstants.BLACK).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE).setFixedLeading(14);
			scadenzaCanvas.add(scadenzaP);
			scadenzaCanvas.close();
		}

		// Puoi Pagare
		Rectangle puoiPagareRectangle = new Rectangle(30, 471, 141, 12); //508
		Canvas puoiPagareCanvas = new Canvas(pdfCanvas, puoiPagareRectangle);
		Text puoiPagareText1 = new Text("Puoi pagare").setFont(asset.getTitillium_regular());
		Text puoiPagareText2 = new Text(" con una unica rata ").setFont(asset.getTitillium_bold());
		Text puoiPagareText3 = new Text(documento.DatiBollettino.size() > 2 ? "oppure in " + Integer.toString(documento.DatiBollettino.size()-1) +" rate (vedi pagina seguente). La rateizzazione non prevede costi aggiuntivi" : "").setFont(asset.getTitillium_regular());
		...
		// Paga Sito
		Rectangle pagaSitoRectangle = new Rectangle(319, 531, 128, 15);
		Canvas pagaSitoCanvas = new Canvas(pdfCanvas, pagaSitoRectangle);
		Text pagaSitoText = new Text("PAGA SUL SITO O CON LE APP").setFont(asset.getTitillium_bold());
		Paragraph pagaSitoP = new Paragraph().add(pagaSitoText).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Paragraph pagaSitoDescriptionP = new Paragraph().add(pagaSitoDescriptionText).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setFixedLeading(8.7f).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		pagaSitoDescriptionCanvas.add(pagaSitoDescriptionP);
		pagaSitoDescriptionCanvas.close();

		// Paga Territorio
		Rectangle pagaTerritorioRectangle = new Rectangle(319, 479, 101, 15);
		Canvas pagaTerritorioCanvas = new Canvas(pdfCanvas, pagaTerritorioRectangle);
		Text pagaTerritorioText = new Text("PAGA SUL  TERRITORIO").setFont(asset.getTitillium_bold());
		...
		Text pagaTerritorioDescriptionText = new Text("In tutti gli Uffici Postali, in Banca, in Ricevitoria, dal Tabaccaio, al Bancomat, al Supermercato. Potrai pagare in contanti, con carte o conto corrente.")
				.setFont(asset.getTitillium_regular());
		Paragraph pagaTerritorioDescriptionP = new Paragraph().add(pagaTerritorioDescriptionText)
				.setFontColor(ColorConstants.BLACK).setFontSize(8).setFixedLeading(8.7f).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text utilizzaPorzioneText1 = new Text("Utilizza la porzione di avviso relativa ").setFont(asset.getTitillium_regular());
		Text utilizzaPorzioneText2 = new Text(documento.DatiBollettino.size() > 2 ? "alla rata " : "").setFont(asset.getTitillium_regular());
		Text utilizzaPorzioneText3 = new Text("al canale di pagamento che preferisci.").setFont(asset.getTitillium_regular());
		Paragraph utilizzaPorzioneP = new Paragraph().add(utilizzaPorzioneText1).add(utilizzaPorzioneText2)
				.add(utilizzaPorzioneText3).setFontColor(ColorConstants.BLACK).setFontSize(11).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Rectangle bancheAltriCanaliRectangle = new Rectangle(29, 390, 106, 15);
		Canvas bancheAltriCanaliCanvas = new Canvas(pdfCanvas, bancheAltriCanaliRectangle);
		Text bancheAltriCanaliText = new Text("BANCHE E ALTRI CANALI").setFont(asset.getTitillium_bold());
		...
		// Rata Unica
		Rectangle rataUnicaRectangle = new Rectangle(416, 390, 89, 15);
		Canvas rataUnicaCanvas = new Canvas(pdfCanvas, rataUnicaRectangle);
		Text rataUnicaText1 = new Text("RATA UNICA").setFont(asset.getTitillium_black());
		Text rataUnicaText2 = new Text(" entro il").setFont(asset.getTitillium_bold());
		...
		// Rata Unica Data
		Rectangle rataUnicadataRectangle = new Rectangle(517, 390, 83, 15);
		Canvas rataUnicadataCanvas = new Canvas(pdfCanvas, rataUnicadataRectangle);
		Text rataUnicadataText = new Text(bollettino999.ScadenzaRata.contains("/") ? bollettino999.ScadenzaRata : "VEDI SOPRA").setFont(asset.getTitillium_bold());
		...
		Text bancheAltriCanaliDescriptionText1 = new Text("Qui accanto trovi il codice")
				.setFont(asset.getTitillium_regular());
		Text bancheAltriCanaliDescriptionText2 = new Text(" QR ").setFont(asset.getTitillium_bold());
		...
		Paragraph bancheAltriCanaliDescriptionP = new Paragraph().add(bancheAltriCanaliDescriptionText1)
				...
				.setFontColor(ColorConstants.BLACK).setFontSize(9).setMargin(0).setFixedLeading(11);
...
// 		QR CODE 
		...
		// Destinatario
		Rectangle destinatarioRectangle = new Rectangle(263, 353, 42, 12);
		Canvas destinatarioCanvas = new Canvas(pdfCanvas, destinatarioRectangle);
		Text destinatarioText = new Text("Destinatario").setFont(asset.getTitillium_regular());
		...
		Rectangle nomeCognomeDestinatarioRectangle2 = new Rectangle(313, 330, 170 - 19 - 25, 32);
		if(bDebug)
		...
		// Ente Creditore
		Rectangle destinatarioRectangle2 = new Rectangle(263, 338, 49, 12);
		Canvas destinatarioCanvas2 = new Canvas(pdfCanvas, destinatarioRectangle2);
		Text destinatarioText2 = new Text("Ente Creditore").setFont(asset.getTitillium_regular());
		...
		Rectangle enteCreditoreStringRectangle2 = new Rectangle(313, 314, 163 - 10, 32);
		if(bDebug)
		...
		// Oggetto Pagamento
		Rectangle oggettoPagamentoRectangle2 = new Rectangle(263, 323, 96, 12);
		...
		Text oggettoPagamentoText2 = new Text("Oggetto del pagamento").setFont(asset.getTitillium_regular());
		...
		Rectangle oggettoPagamentoStringRectangle2 = new Rectangle(346, 301 - 2 -2, 239 - 20 - 15 -6, 32); 
		if(bDebug)
		...
		//		.setVerticalAlignment(VerticalAlignment.MIDDLE);
		Paragraph oggettoPagamentoStringP2 = new Paragraph().add(oggettoPagamentoText)
				.setFontColor(ColorConstants.BLACK).setFontSize(8).setMargin(0).setFixedLeading(7)
				.setVerticalAlignment(VerticalAlignment.MIDDLE);
		//fine LP PG200420 - Errori nel pdf dell'avviso
		...
		Text euroText = new Text("Euro").setFont(asset.getTitillium_regular());
		Paragraph euroP = new Paragraph().add(euroText).setFontSize(10).setFontColor(ColorConstants.BLACK).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.MIDDLE);
		euroCanvas.add(euroP);
		euroCanvas.close();

		// Importo
		Rectangle importoRectangle2 = new Rectangle(510, 352, 60, 15);
		Canvas importoCanvas2 = new Canvas(pdfCanvas, importoRectangle2);
		Paragraph importoP2 = new Paragraph().add(importoText).setFontColor(ColorConstants.BLACK).setFontSize(11)
		...
		// Codice Cbill
		Rectangle codiceCbillRectangle = new Rectangle(263, 301, 43, 12);
		Canvas codiceCbillCanvas = new Canvas(pdfCanvas, codiceCbillRectangle);
		Text codiceCbillText = new Text("Codice CBILL").setFont(asset.getTitillium_regular());
		...
		// Codice Avviso
		Rectangle codiceAvvisoRectangle = new Rectangle(325, 301, 49, 12);
		Canvas codiceAvvisoCanvas = new Canvas(pdfCanvas, codiceAvvisoRectangle);
		Text codiceAvvisoText = new Text("Codice Avviso").setFont(asset.getTitillium_regular());
		...
		Text codiceAvvisoStringText = new Text(bollettino999.AvvisoPagoPa).setFont(asset.getRoboto_bold());
		...
		Rectangle cfEnteCreditoreRectangle = new Rectangle(475, 301, 90, 12);
		Canvas cfEnteCreditoreCanvas = new Canvas(pdfCanvas, cfEnteCreditoreRectangle);
		Text cfEnteCreditoreText = new Text("Cod. Fiscale Ente Creditore").setFont(asset.getTitillium_regular());
		...
		if(tipoStampa.equals("P")) {
			...
			Rectangle bollettinoPostalePaRectangle = new Rectangle(29, 247, 115, 15);
			Canvas bollettinoPostalePaCanvas = new Canvas(pdfCanvas, bollettinoPostalePaRectangle);
			Text bollettinoPostalePaText = new Text(LeggoAsset.BOLLETTINO_POSTALE_PA).setFont(asset.getTitillium_bold());
			...
			// Rata Unica
			Rectangle rataUnicaRectangle2 = new Rectangle(416, 247, 89, 15);
			...
			Text sulCcText = new Text(LeggoAsset.SUL_CC).setFont(asset.getTitillium_regular());
			Paragraph sulCcP = new Paragraph().add(sulCcText).setFontColor(ColorConstants.BLACK).setFontSize(10)
					.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
			sulCcCanvas.add(sulCcP);
			sulCcCanvas.close();
	
			// Numero CC Postale
			Rectangle numeroCcPostaleRectangle = new Rectangle(248, 193, 156, 15);
			...
			Paragraph numeroCcPostaleP = new Paragraph().add(numeroCcPostaleText).setFontColor(ColorConstants.BLACK)
					.setFontSize(11).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
			...
			// Importo
			Rectangle importoRectangle3 = new Rectangle(510, 195, 60, 15);
			...
			// Intestato a
			Rectangle intestatoARectangle = new Rectangle(178, 163, 39, 12);
			Canvas intestatoACanvas = new Canvas(pdfCanvas, intestatoARectangle);
			Text intestatoAText = new Text(LeggoAsset.INTESTATO_A).setFont(asset.getTitillium_regular());
			...
			Rectangle intestatarioCCPostaleRectangle = new Rectangle(220, 164, 258, 10);
			//fine LP PG200420 - Errori nel pdf dell'avviso
			Canvas intestatarioCCPostaleCanvas = new Canvas(pdfCanvas, intestatarioCCPostaleRectangle);
			Text intestatarioCCPostaleText = new Text(bollettino999.Descon60Boll).setFont(asset.getRoboto_bold());
			...
			// Destinatario
			Rectangle destinatarioRectangle3 = new Rectangle(178, 145, 47, 12);
			...
			Rectangle nomeCognomeDestinatarioRectangle3 = new Rectangle(226, 122, 170 - 10 - 9, 32);
			if(bDebug) //luis
			...
			// Oggetto Pagamento
			Rectangle oggettoPagamentoRectangle3 = new Rectangle(178, 128, 68, 12);
			oggettoPagamentoRectangle3 = new Rectangle(178, 128 - 2, 68, 12);
			Canvas oggettoPagamentoCanvas3 = new Canvas(pdfCanvas, oggettoPagamentoRectangle3);
			Text oggettoPagamentoText3 = new Text(LeggoAsset.OGGETTO_PAGAMENTO).setFont(asset.getTitillium_regular());
			...
			// Oggetto Pagamento String
			Rectangle oggettoPagamentoStringRectangle3 = new Rectangle(250, 109 - 2 -2, 239 - 8, 30);
			//inizio LP PG200420 - Errori nel pdf dell'avviso
			if(bDebug)
			...
			Text oggettoPagamentoStringText3 = new Text(documento.CausaleDocumento).setFont(asset.getRoboto_bold());
			//inizio LP PG200420 - Errori nel pdf dell'avviso
	...
	// 		Data matrix
			...
			// Codice Avviso
			Rectangle codiceAvvisoRectangle2 = new Rectangle(179, 103, 49, 12);
			...
			// Tipo
			Rectangle tipoRectangle = new Rectangle(329, 103, 15, 12);
			Canvas tipoCanvas = new Canvas(pdfCanvas, tipoRectangle);
			Text tipoText = new Text(LeggoAsset.TIPO).setFont(asset.getTitillium_regular());
			...
			// Cod. Fiscale Ente Creditore
			Rectangle cfEnteCreditoreRectangle2 = new Rectangle(375, 103, 90, 12);
		...
		//Nota
		Rectangle notaRettangolo = new Rectangle(38, 30, 400, 15);
		Canvas notaCanvas = new Canvas(pdfCanvas, notaRettangolo);
		Text notaText = new Text("Questo bollettino può essere utilizzato sul circuito pagoPA fino al " + (bollettino999.scadenzaBollettino==null ? bollettino999.ScadenzaRata : bollettino999.scadenzaBollettino) + ".").setFont(asset.getTitillium_regular());
		Paragraph notaParagrafo = new Paragraph().add(notaText)
				.setFontColor(ColorConstants.BLACK).setFontSize(11).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.MIDDLE);
		notaCanvas.add(notaParagrafo);
		notaCanvas.close();
...
//	 	Avviso di Pagamento
		Rectangle avvisoPagamentoRectangle = new Rectangle(70, 798, 104, 15);
		Canvas avvisoPagamentoCanvas = new Canvas(pdfCanvas, avvisoPagamentoRectangle);
		Text avvisoPagamentoText = new Text("AVVISO DI PAGAMENTO").setFont(asset.getTitillium_bold());
		Paragraph avvisoPagamentoP = new Paragraph().add(avvisoPagamentoText).setFontColor(ColorConstants.BLACK)
				.setFontSize(10).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
...
// 		TESTO UTILIZZA LA PORZIONE...
		...
		Text b1_2utilizzaLaPorzioneText = new Text("Utilizza la porzione di avviso relativa alla rata ed al canale di pagamento che preferisci.").setFont(asset.getTitillium_regular());
...
// 		BANCHE ED ALTRI
		...
		Text b1sxBancaEAltriText = new Text("BANCHE E ALTRI").setFont(asset.getTitillium_bold());
...
// 		RATA E SCADENZA
		...
		Text b1sxScadenzaText = new Text("entro il " + documento.DatiBollettino.get(bollettinoDiPartenza).ScadenzaRata).setFont(asset.getTitillium_bold())
				.setFontSize(10);
		Paragraph b1sxNumRataEScadenzaParagrafo = new Paragraph().add(b1sxNumRataText).add(b1sxScadenzaText)
				.setFontColor(ColorConstants.BLACK).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
...
//		TESTO EURO e IMPORTO
		...
		Text b1sxEuroTesto = new Text("Euro  ").setFont(asset.getTitillium_regular()).setFontSize(10);
		Text b1sxImportTesto = new Text(
				mettiVirgolaEPuntiAllImportoInCent(documento.DatiBollettino.get(bollettinoDiPartenza).Codeline2Boll))
						.setFont(asset.getRoboto_bold()).setFontSize(12.24f);
...
//		TESTO DESTINATARIO AVVISO
		...
		Text b1sxValoreDestinatarioAvvisoTesto = new Text(documento.DatiAnagrafici.get(0).Denominazione1)
				.setFont(asset.getTitillium_bold()).setFontSize(8);
...
//		TESTO OGGETTO DEL PAGAMENTO
		...
		Paragraph b1sxOggettoDelPagamentoParagrafo = new Paragraph().add(b1sxOggettoDelPagamentoTesto)
				.add(b1sxValoreOggettoDelPagamentoTesto).setFontColor(ColorConstants.BLACK).setFontSize(8).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.BOTTOM).setFixedLeading(12);
...
//		TESTO CBILL
		...
		Text b1sxValoreCbillTesto = new Text(documento.DatiCreditore.get(0).CodiceInterbancario).setFont(asset.getRoboto_bold())
				.setFontSize(10);
...
//		TESTO CODICE FISCALE ENTE CREDITORE
		...
		Text b1sxValoreCodFiscEnteCredTesto = new Text(documento.DatiCreditore.get(0).Cf)
				.setFont(asset.getRoboto_bold()).setFontSize(10);
		Paragraph b1sxCodFiscEnteCredParagrafo = new Paragraph().add(b1sxCodFiscEnteCredTesto)
				.add(b1sxValoreCodFiscEnteCredTesto).setFontColor(ColorConstants.BLACK).setFontSize(8).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.BOTTOM).setFixedLeading(11.4f);
...
//		TESTO CODICE AVVISO 
		...
		Text b1sxValoreCBillTesto = new Text(documento.DatiBollettino.get(bollettinoDiPartenza).AvvisoPagoPa).setFont(asset.getRoboto_bold())
				.setFontSize(10);
		Paragraph b1sxCBillParagrafo = new Paragraph().add(b1sxCBillTesto).add(b1sxValoreCBillTesto)
...
//		TESTO ISTRUZIONI BANCHE E ALTRI 
		...
		Text b1sxIstruzioniBancheAltro1Testo = new Text("Qui sopra trovi il codice ")
				.setFont(asset.getTitillium_regular());
		Text b1sxQRIstruzioniBancheAltroTesto = new Text("QR").setFont(asset.getRoboto_bold());
		Text b1sxIstruzioniBancheAltro2Testo = new Text(" e il codice interbancario ")
				.setFont(asset.getTitillium_regular());
		Text b1sxCbillIstruzioniBancheAltroTesto = new Text("CBILL").setFont(asset.getRoboto_bold());
...
// 		BANCHE ED ALTRI
		...
		Text b2dxBancaEAltriText = new Text("BANCHE E ALTRI").setFont(asset.getTitillium_bold());
...
// 		RATA E SCADENZA
		...
		Text b2dxScadenzaText = new Text("entro il " + documento.DatiBollettino.get(bollettinoDiPartenza + 1).ScadenzaRata).setFont(asset.getTitillium_bold())
				.setFontSize(9.8f);
		Paragraph b2dxNumRataEScadenzaParagrafo = new Paragraph().add(b2dxNumRataText).add(b2dxScadenzaText)
				.setFontColor(ColorConstants.BLACK).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
...
//		TESTO EURO e IMPORTO
		...
		Text b2dxEuroTesto = new Text("Euro  ").setFont(asset.getTitillium_regular()).setFontSize(10);
...
//		TESTO CBILL
		...
		Text b2dxValoreCbillTesto = new Text(documento.DatiCreditore.get(0).CodiceInterbancario).setFont(asset.getRoboto_bold())
				.setFontSize(10);
...
//		TESTO CODICE AVVISO 
		...
		Text b2dxValoreCBillTesto = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 1).AvvisoPagoPa).setFont(asset.getRoboto_bold())
				.setFontSize(10);
		Paragraph b2dxCBillParagrafo = new Paragraph().add(b2dxCBillTesto).add(b2dxValoreCBillTesto)
				.setFontColor(ColorConstants.BLACK).setMargin(0).setVerticalAlignment(VerticalAlignment.BOTTOM)
				.setFixedLeading(10.4f);
...
//		TESTO ISTRUZIONI BANCHE E ALTRI 
		...
		Text b2dxIstruzioniBancheAltro1Testo = new Text("Qui sopra trovi il codice ")
				.setFont(asset.getTitillium_regular());
		Text b2dxQRIstruzioniBancheAltroTesto = new Text("QR").setFont(asset.getRoboto_bold());
		Text b2dxIstruzioniBancheAltro2Testo = new Text(" e il codice interbancario ")
				.setFont(asset.getTitillium_regular());
		Text b2dxCbillIstruzioniBancheAltroTesto = new Text("CBILL").setFont(asset.getRoboto_bold());
...
//		TESTO BOLLETTINO POSTALE PA
		Rectangle b1_2rettangoloTestoBollettinoPostePA = new Rectangle(31, 450, 115, 17);
		Canvas b1_2TestoBollettinoPostePACanvas = new Canvas(pdfCanvas, b1_2rettangoloTestoBollettinoPostePA);
		Text b1_2intest = new Text("BOLLETTINO POSTALE PA").setFont(asset.getTitillium_bold());
...
//		TESTO N° RATA E SCADENZA
		...
		Paragraph b1_2paragrafoNumEScadRata = new Paragraph().add(b1_2NumRataTesto).setFontColor(ColorConstants.BLACK)
				.setFontSize(10).add(b1_2entroIlTesto).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.add(b1_2scadenzaTesto).setFontColor(ColorConstants.BLACK).setFontSize(10).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.MIDDLE);
...
//		TESTO SUL CC e VALORE NUM CC
		...
		Text b1_2CCTesto = new Text("sul C/C n. ").setFont(asset.getTitillium_regular()).setFontSize(10);
		Text b1_2numCcPostTesto = new Text(documento.DatiBollettino.get(bollettinoDiPartenza).Codeline12Boll).setFont(asset.getRoboto_bold()).setFontSize(12.24f);//numero cc postale
...
//		TESTO EURO e IMPORTO
		...
		Text b1_2EuroTesto = new Text("Euro  ").setFont(asset.getTitillium_regular()).setFontSize(10);
		Text b1_2ImportTesto = b1sxImportTesto.setFont(asset.getRoboto_bold()).setFontSize(12.24f);
...
//		TESTO DOVE PAGARE
		...
		Text b1_2dovePagareTesto = new Text("Bollettino Postale pagabile in tutti gli Uffici Postali e sui canali fisici o digitali abilitati di Poste italiane e dell'Ente Creditore").setFont(asset.getTitillium_regular());
		Paragraph b1_2paragrafoDovePagare = new Paragraph().add(b1_2dovePagareTesto).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setMargin(0).setVerticalAlignment(VerticalAlignment.TOP).setFixedLeading(9.6f);
...
//		TESTO AUTORIZZAZIONE
		...
		Text b1_2AutorizzazioneTesto = new Text(documento.DatiBollettino.get(bollettinoDiPartenza).AutorizCcp).setFont(asset.getTitillium_regular());
		Paragraph b1_2paragrafoAutorizzazione = new Paragraph().add(b1_2AutorizzazioneTesto)
				.setFontColor(ColorConstants.BLACK).setFontSize(6).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.TOP);
		b1_2AutorizzazioneCanvas.add(b1_2paragrafoAutorizzazione);
		b1_2AutorizzazioneCanvas.close();

		// Intestato a
		Rectangle b1_intestatoARectangle = new Rectangle(178, 362, 39, 12);
		Canvas b1_intestatoACanvas = new Canvas(pdfCanvas, b1_intestatoARectangle);
		Text b1_intestatoAText = new Text(LeggoAsset.INTESTATO_A).setFont(asset.getTitillium_regular());
		...
		// Intestatario CC Postale
		Rectangle b1_intestatarioCCPostaleRectangle = new Rectangle(220, 363, 258, 10);
		...
		// Destinatario		
		Rectangle b1_destinatarioRectangle3 = new Rectangle(178, 345, 47, 12);
		Canvas b1_destinatarioCanvas3 = new Canvas(pdfCanvas, b1_destinatarioRectangle3);
		Text b1_destinatarioText = new Text("Destinatario").setFont(asset.getTitillium_regular());
		...
		Rectangle b1_nomeDestinatarioRectangle3 = new Rectangle(229, 322, 250, 32);
		if(bDebug)
		...
		// Oggetto Pagamento
		Rectangle b1_oggettoPagamentoRectangle3 = new Rectangle(178, 330, 68, 12);
		Canvas b1_oggettoPagamentoCanvas3 = new Canvas(pdfCanvas, b1_oggettoPagamentoRectangle3);
		Text b1_oggettoPagamentoText3 = new Text(LeggoAsset.OGGETTO_PAGAMENTO).setFont(asset.getTitillium_regular());
		...
		// Oggetto Pagamento String
		Rectangle b1_oggettoPagamentoStringRectangle3 = new Rectangle(250, 310, 213, 30);
		//inizio LP PG200420 - Errori nel pdf dell'avviso
		if(bDebug)
		...
		Text b1_oggettoPagamentoStringText3 = new Text(documento.CausaleDocumento).setFont(asset.getRoboto_bold());
...
//		TESTO CODICE AVVISO
		...
		Text b1_2valoreCodiceAvvisoTesto = new Text(documento.DatiBollettino.get(bollettinoDiPartenza).AvvisoPagoPa).setFont(asset.getRoboto_bold())
				.setFontSize(10.2f);
		Paragraph b1_2paragrafoCodiceAvviso = new Paragraph().add(b1_2codiceAvvisoTesto)
				.add(b1_2valoreCodiceAvvisoTesto).setFontColor(ColorConstants.BLACK).setFontSize(8).setMargin(0)
...
//		TESTO TIPO
		Rectangle b1_2rettangoloTipo = new Rectangle(333, 284, 16, 30);
		Canvas b1_2tipoCanvas = new Canvas(pdfCanvas, b1_2rettangoloTipo);
		Text b1_2tipoTesto = new Text("Tipo\n").setFont(asset.getTitillium_regular()).setFontSize(8);
		Text b1_2tipoValoreTesto = new Text(LeggoAsset.P1).setFont(asset.getRoboto_bold()).setFontSize(10.2f);
...
//		TESTO COD FISC ENTE CREDITORE
		...
		Text b1_2CodFiscEnteCredValoreTesto = new Text(documento.DatiCreditore.get(0).Cf)
				.setFont(asset.getRoboto_bold()).setFontSize(10.2f);
		Paragraph b1_2paragrafoCodFiscEnteCred = new Paragraph().add(b1_2CodFiscEnteCredTesto)
				.add(b1_2CodFiscEnteCredValoreTesto).setFontColor(ColorConstants.BLACK).setFontSize(8).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.BOTTOM).setFixedLeading(10.2f);
...
//		TESTO BOLLETTINO POSTALE PA
		Rectangle b2_2rettangoloTestoBollettinoPostePA = new Rectangle(31, 226, 115, 17);
		Canvas b2_2TestoBollettinoPostePACanvas = new Canvas(pdfCanvas, b2_2rettangoloTestoBollettinoPostePA);
		Text b2_2intest = new Text("BOLLETTINO POSTALE PA").setFont(asset.getTitillium_bold());
...
//		TESTO N° RATA E SCADENZA
		...
		Paragraph b2_2paragrafoNumEScadRata = new Paragraph().add(b2_2NumRataTesto).setFontColor(ColorConstants.BLACK)
				.setFontSize(10).add(b2_2entroIlTesto).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.add(b2_2scadenzaTesto).setFontColor(ColorConstants.BLACK).setFontSize(10).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.MIDDLE);
...
//		TESTO SUL CC e VALORE NUM CC
		...
		Text b2_2CCTesto = new Text("sul C/C n. ").setFont(asset.getTitillium_regular()).setFontSize(10);
		Text b2_2numCcPostTesto = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 1).Codeline12Boll).setFont(asset.getRoboto_bold())
				.setFontSize(12.24f);
...
//		TESTO EURO e IMPORTO
		...
		Text b2_2EuroTesto = new Text("Euro  ").setFont(asset.getTitillium_regular()).setFontSize(10);
...
//		TESTO DOVE PAGARE
		...
		Text b2_2dovePagareTesto = new Text("Bollettino Postale pagabile in tutti gli Uffici Postali e sui canali fisici o digitali abilitati di Poste italiane e dell'Ente Creditore").setFont(asset.getTitillium_regular());
...
//		TESTO AUTORIZZAZIONE
		...
		Text b2_2AutorizzazioneTesto = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 1).AutorizCcp).setFont(asset.getTitillium_regular());
		...
		// Intestato a
		Rectangle b2_intestatoARectangle = new Rectangle(178, 362 - 225, 39, 12);
		Canvas b2_intestatoACanvas = new Canvas(pdfCanvas, b2_intestatoARectangle);
		Text b2_intestatoAText = new Text(LeggoAsset.INTESTATO_A).setFont(asset.getTitillium_regular());
		...
		// Intestatario CC Postale
		Rectangle b2_intestatarioCCPostaleRectangle = new Rectangle(220, 363 - 225, 258, 10);
		...
		// Destinatario
		Rectangle b2_destinatarioRectangle3 = new Rectangle(178, 345 - 225, 47, 12);
		Canvas b2_destinatarioCanvas3 = new Canvas(pdfCanvas, b2_destinatarioRectangle3);
		Text b2_destinatarioText = new Text("Destinatario").setFont(asset.getTitillium_regular());
		...
		Rectangle b2_nomeDestinatarioRectangle3 = new Rectangle(229, 322 - 225, 250, 32);
		//fine LP PG200420 - Errori nel pdf dell'avviso
		...
		// Oggetto Pagamento
		Rectangle b2_oggettoPagamentoRectangle3 = new Rectangle(178, 330 - 225, 68, 12);
		Canvas b2_oggettoPagamentoCanvas3 = new Canvas(pdfCanvas, b2_oggettoPagamentoRectangle3);
		Text b2_oggettoPagamentoText3 = new Text(LeggoAsset.OGGETTO_PAGAMENTO).setFont(asset.getTitillium_regular());
		...
		// Oggetto Pagamento String
		Rectangle b2_oggettoPagamentoStringRectangle3 = new Rectangle(250, 310 - 225, 213, 30);
		Canvas b2_oggettoPagamentoStringCanvas3 = new Canvas(pdfCanvas, b2_oggettoPagamentoStringRectangle3);
		Text b2_oggettoPagamentoStringText3 = new Text(documento.CausaleDocumento).setFont(asset.getRoboto_bold());
...
//		TESTO CODICE AVVISO
		...
		Text b2_2valoreCodiceAvvisoTesto = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 1).AvvisoPagoPa).setFont(asset.getRoboto_bold())
				.setFontSize(10.2f);
		Paragraph b2_2paragrafoCodiceAvviso = new Paragraph().add(b2_2codiceAvvisoTesto)
				.add(b2_2valoreCodiceAvvisoTesto).setFontColor(ColorConstants.BLACK).setFontSize(8).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.BOTTOM).setFixedLeading(10.2f);
		b2_2codiceAvvisoCanvas.add(b2_2paragrafoCodiceAvviso);
		b2_2codiceAvvisoCanvas.close();
//		TESTO TIPO
		Rectangle b2_2rettangoloTipo = new Rectangle(333, 60, 16, 30);
		Canvas b2_2tipoCanvas = new Canvas(pdfCanvas, b2_2rettangoloTipo);
		Text b2_2tipoTesto = new Text("Tipo\n").setFont(asset.getTitillium_regular()).setFontSize(8);
		Text b2_2tipoValoreTesto = new Text(LeggoAsset.P1).setFont(asset.getRoboto_bold()).setFontSize(10.2f);
...
//		TESTO COD FISC ENTE CREDITORE
		...
		Paragraph b2_2paragrafoCodFiscEnteCred = new Paragraph().add(b2_2CodFiscEnteCredTesto)
				.add(b2_2CodFiscEnteCredValoreTesto).setFontColor(ColorConstants.BLACK).setFontSize(8).setMargin(0)
				.setVerticalAlignment(VerticalAlignment.BOTTOM).setFixedLeading(10.2f);
...
//		creazione del pdf canvas
		...
		Canvas utilizzaPorzioneCanvas = new Canvas(pdfCanvas, utilizzaPorzioneRectangle);
		Text utilizzaPorzioneText = new Text("Utilizza la porzione di avviso relativa alla rata ed al canale di pagamento che preferisci.").setFont(asset.getTitillium_regular());
		Paragraph utilizzaPorzioneP = new Paragraph().add(utilizzaPorzioneText).setFontColor(ColorConstants.BLACK)
				.setFontSize(9).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		utilizzaPorzioneCanvas.add(utilizzaPorzioneP);
		utilizzaPorzioneCanvas.close();
		...
		// qR3Canvas.add(asset.getQr_code().scaleToFit(70, 70));
		// qR3Canvas.close();
...
// 		QR CODE 3
		...
		Text importoText = new Text(mettiVirgolaEPuntiAllImportoInCent(documento.DatiBollettino.get(bollettinoDiPartenza).Codeline2Boll))
				.setFont(asset.getRoboto_bold());
		Paragraph importoP = new Paragraph().add(importoText).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		importoCanvas.add(importoP);
		importoCanvas.close();
		...
		Canvas enteCreditoreCanvas = new Canvas(pdfCanvas, enteCreditoreRectangle);
		Text enteCreditoreText = new Text("Ente Creditore").setFont(asset.getTitillium_regular());
		Paragraph enteCreditoreP = new Paragraph().add(enteCreditoreText).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		enteCreditoreCanvas.add(enteCreditoreP);
		enteCreditoreCanvas.close();
		...
		Text enteCreditoreStringText = new Text(documento.DatiCreditore.get(0).Denominazione1)
				.setFont(asset.getTitillium_bold());
		Paragraph enteCreditoreStringP = new Paragraph().add(enteCreditoreStringText).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE).setFixedLeading(7);
		enteCreditoreStringCanvas.add(enteCreditoreStringP);
		enteCreditoreStringCanvas.close();

		// Oggetto Pagamento
		Rectangle oggettoPagamentoRectangle2 = new Rectangle(30, 598, 80, 12);
		Canvas oggettoPagamentoCanvas2 = new Canvas(pdfCanvas, oggettoPagamentoRectangle2);
		//inizio LP PG200420 
		...
		//fine LP PG200420 - Errori nel pdf dell'avviso
		Canvas oggettoPagamentoStringCanvas2 = new Canvas(pdfCanvas, oggettoPagamentoStringRectangle2);
		Text oggettoPagamentoStringText2 = new Text(documento.CausaleDocumento).setFont(asset.getTitillium_bold());
		...
		// Codice Cbill
		Rectangle codiceCbillRectangle = new Rectangle(30, 560, 43, 12);
		Canvas codiceCbillCanvas = new Canvas(pdfCanvas, codiceCbillRectangle);
		Text codiceCbillText = new Text("Codice CBILL").setFont(asset.getTitillium_regular());
		Paragraph codiceCbillP = new Paragraph().add(codiceCbillText).setFontColor(ColorConstants.BLACK).setFontSize(8)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		// Cod. Fiscale Ente Creditore
		Rectangle cfEnteCreditoreRectangle = new Rectangle(90, 560, 90, 12);
		Canvas cfEnteCreditoreCanvas = new Canvas(pdfCanvas, cfEnteCreditoreRectangle);
		Text cfEnteCreditoreText = new Text("Cod. Fiscale Ente Creditore").setFont(asset.getTitillium_regular());
		Paragraph cfEnteCreditoreP = new Paragraph().add(cfEnteCreditoreText).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		// Codice Avviso
		Rectangle codiceAvvisoRectangle = new Rectangle(30, 530, 49, 12);
		Canvas codiceAvvisoCanvas = new Canvas(pdfCanvas, codiceAvvisoRectangle);
		Text codiceAvvisoText = new Text("Codice Avviso").setFont(asset.getTitillium_regular());
		Paragraph codiceAvvisoP = new Paragraph().add(codiceAvvisoText).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Canvas codiceAvvisoStringCanvas = new Canvas(pdfCanvas, codiceAvvisoStringRectangle);
		Text codiceAvvisoStringText = new Text(documento.DatiBollettino.get(bollettinoDiPartenza).AvvisoPagoPa).setFont(asset.getRoboto_bold());
		Paragraph codiceAvvisoStringP = new Paragraph().add(codiceAvvisoStringText).setFontColor(ColorConstants.BLACK)
				.setFontSize(10).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		codiceAvvisoStringCanvas.add(codiceAvvisoStringP);
		codiceAvvisoStringCanvas.close();
		...
		Paragraph bancheAltriCanaliDescriptionP = new Paragraph().add(bancheAltriCanaliDescriptionText1)
				.add(bancheAltriCanaliDescriptionText2).add(bancheAltriCanaliDescriptionText3)
				.add(bancheAltriCanaliDescriptionText4).add(bancheAltriCanaliDescriptionText5)
				.setFontColor(ColorConstants.BLACK).setFontSize(7).setMargin(0).setFixedLeading(9);
		bancheAltriCanaliDescriptionCanvas.add(bancheAltriCanaliDescriptionP);
		bancheAltriCanaliDescriptionCanvas.close();
...
// 		QR 4
		...
		Text importoText2 = new Text(mettiVirgolaEPuntiAllImportoInCent(documento.DatiBollettino.get(bollettinoDiPartenza + 1).Codeline2Boll))
				.setFont(asset.getRoboto_bold());
		Paragraph importoP2 = new Paragraph().add(importoText2).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		importoCanvas2.add(importoP2);
		importoCanvas2.close();
		...
		// Oggetto Pagamento
		Rectangle oggettoPagamentoRectangle3 = new Rectangle(221, 598, 80, 12);
		Canvas oggettoPagamentoCanvas3 = new Canvas(pdfCanvas, oggettoPagamentoRectangle3);
		...
		// Codice Cbill
		Rectangle codiceCbillRectangle2 = new Rectangle(221, 560, 43, 12);
		...
		// Cod. Fiscale Ente Creditore
		Rectangle cfEnteCreditoreRectangle2 = new Rectangle(280, 560, 90, 12);
		Canvas cfEnteCreditoreCanvas2 = new Canvas(pdfCanvas, cfEnteCreditoreRectangle2);
		cfEnteCreditoreCanvas2.add(cfEnteCreditoreP);
		cfEnteCreditoreCanvas2.close();
		...
		// Codice Avviso
		Rectangle codiceAvvisoRectangle2 = new Rectangle(221, 530, 49, 12);
		Canvas codiceAvvisoCanvas2 = new Canvas(pdfCanvas, codiceAvvisoRectangle2);
		codiceAvvisoCanvas2.add(codiceAvvisoP);
		codiceAvvisoCanvas2.close();
		...
		Canvas codiceAvvisoStringCanvas2 = new Canvas(pdfCanvas, codiceAvvisoStringRectangle2);
		Text codiceAvvisoStringText2 = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 1).AvvisoPagoPa).setFont(asset.getRoboto_bold());
...
// 		QR 5
		...
		Text importoText3 = new Text(mettiVirgolaEPuntiAllImportoInCent(documento.DatiBollettino.get(bollettinoDiPartenza + 2).Codeline2Boll))
				.setFont(asset.getRoboto_bold());
		Paragraph importoP3 = new Paragraph().add(importoText3).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		// Oggetto Pagamento
		Rectangle oggettoPagamentoRectangle4 = new Rectangle(420, 598, 80, 12);
		...
		// Codice Cbill
		Rectangle codiceCbillRectangle3 = new Rectangle(420, 560, 43, 12);
		...
		// Cod. Fiscale Ente Creditore
		Rectangle cfEnteCreditoreRectangle3 = new Rectangle(478, 560, 90, 12);
		...
		// Codice Avviso
		Rectangle codiceAvvisoRectangle3 = new Rectangle(420, 530, 49, 12);
		...
		Text codiceAvvisoStringText3 = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 2).AvvisoPagoPa).setFont(asset.getRoboto_bold());
		...
		// Bollettino Postale PA
		Rectangle b1_bollettinoPostalePaRectangle = new Rectangle(29, 462, 115, 15);
		Canvas b1_bollettinoPostalePaCanvas = new Canvas(pdfCanvas, b1_bollettinoPostalePaRectangle);
		Text b1_bollettinoPostalePaText = new Text(LeggoAsset.BOLLETTINO_POSTALE_PA).setFont(asset.getTitillium_bold());
		...
		Paragraph b1_numeroRataP = new Paragraph().add(b1_numeroRataText1).add(b1_numeroRataText2)
				.add(b1_numeroRataText3).add(b1_numeroRataText4).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text b1_suCcText = new Text(LeggoAsset.SUL_CC).setFont(asset.getTitillium_regular());
		Paragraph b1_suCcP = new Paragraph().add(b1_suCcText).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text b1_numeroCcPostaleText = new Text(documento.DatiBollettino.get(bollettinoDiPartenza).Codeline12Boll).setFont(asset.getTitillium_bold());
		...
		// Autorizzazione
		Rectangle b1_autorizzazioneRectangle = new Rectangle(30, 327, 160, 9);
		Canvas b1_autorizzazioneCanvas = new Canvas(pdfCanvas, b1_autorizzazioneRectangle);
		Text b1_autorizzazioneText = new Text(documento.DatiBollettino.get(bollettinoDiPartenza).AutorizCcp).setFont(asset.getTitillium_regular());
		...
		// Intestato a
		Rectangle b1_intestatoARectangle = new Rectangle(178, 406, 39, 12);
		Canvas b1_intestatoACanvas = new Canvas(pdfCanvas, b1_intestatoARectangle);
		Text b1_intestatoAText = new Text(LeggoAsset.INTESTATO_A).setFont(asset.getTitillium_regular());
		Paragraph b1_intestatoAP = new Paragraph().add(b1_intestatoAText).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Rectangle b1_intestatarioCCPostaleRectangle = new Rectangle(220, 407, 258, 10);
		//inizio LP PG200420 - Errori nel pdf dell'avviso
		Canvas b1_intestatarioCCPostaleCanvas = new Canvas(pdfCanvas, b1_intestatarioCCPostaleRectangle);
		Text b1_intestatarioCCPostaleText = new Text(documento.DatiBollettino.get(bollettinoDiPartenza).Descon60Boll)
				.setFont(asset.getRoboto_bold());
		...
		// Destinatario
		Rectangle b1_destinatarioRectangle3 = new Rectangle(178, 389, 47, 12);
		Canvas b1_destinatarioCanvas3 = new Canvas(pdfCanvas, b1_destinatarioRectangle3);
		Text b1_destinatarioText = new Text("Destinatario").setFont(asset.getTitillium_regular());
		Paragraph b1_destinatarioP = new Paragraph().add(b1_destinatarioText).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Rectangle b1_nomeDestinatarioRectangle3 = new Rectangle(226, 366, 170 - 10 - 9 - 25, 32);
		if(bDebug)
			pdfCanvas.saveState().setFillColor(new DeviceRgb(25, 0, 225)).rectangle(b1_nomeDestinatarioRectangle3).fill();
		//fine LP PG200420 - Errori nel pdf dell'avviso
		Canvas b1_nomeDestinatarioCanvas3 = new Canvas(pdfCanvas, b1_nomeDestinatarioRectangle3);
		...
		// Oggetto Pagamento
		Rectangle b1_oggettoPagamentoRectangle3 = new Rectangle(178, 371, 68, 12);
		Canvas b1_oggettoPagamentoCanvas3 = new Canvas(pdfCanvas, b1_oggettoPagamentoRectangle3);
		Text b1_oggettoPagamentoText3 = new Text(LeggoAsset.OGGETTO_PAGAMENTO).setFont(asset.getTitillium_regular());
		Paragraph b1_oggettoPagamentoP3 = new Paragraph().add(b1_oggettoPagamentoText3)
				.setFontColor(ColorConstants.BLACK).setFontSize(8).setMargin(0)
		...
		// Oggetto Pagamento String
		Rectangle b1_oggettoPagamentoStringRectangle3 = new Rectangle(250, 350, 213, 30);
		//inizio LP PG200420 - Errori nel pdf dell'avviso
		if(bDebug)
			pdfCanvas.saveState().setFillColor(new DeviceRgb(225, 0, 25)).rectangle(b1_oggettoPagamentoStringRectangle3).fill();
		//fine LP PG200420 - Errori nel pdf dell'avviso
...
// 		Data matrix
		...
		// Codice Avviso
		Rectangle b1_codiceAvvisoRectangle2 = new Rectangle(179, 348, 49, 12);
		...
		// Tipo
		Rectangle b1_tipoRectangle = new Rectangle(329, 348, 15, 12);
		Canvas b1_tipoCanvas = new Canvas(pdfCanvas, b1_tipoRectangle);
		Text b1_tipoText = new Text(LeggoAsset.TIPO).setFont(asset.getTitillium_regular());
		Paragraph b1_tipoP = new Paragraph().add(b1_tipoText).setFontColor(ColorConstants.BLACK).setFontSize(8)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text b1_p1Text = new Text(LeggoAsset.P1).setFont(asset.getTitillium_bold());
		Paragraph b1_p1P = new Paragraph().add(b1_p1Text).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		b1_p1Canvas.add(b1_p1P);
		b1_p1Canvas.close();

		// Cod. Fiscale Ente Creditore
		Rectangle b1_cfEnteCreditoreRectangle2 = new Rectangle(375, 348, 90, 12);
		...
		// Bollettino Postale PA
		Rectangle b2_bollettinoPostalePaRectangle = new Rectangle(29, 462 - 160, 115, 15);
		Canvas b2_bollettinoPostalePaCanvas = new Canvas(pdfCanvas, b2_bollettinoPostalePaRectangle);
		Text b2_bollettinoPostalePaText = new Text(LeggoAsset.BOLLETTINO_POSTALE_PA).setFont(asset.getTitillium_bold());
		...
		Paragraph b2_numeroRataP = new Paragraph().add(b2_numeroRataText1).add(b2_numeroRataText2)
				.add(b2_numeroRataText3).add(b2_numeroRataText4).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text b2_suCcText = new Text(LeggoAsset.SUL_CC).setFont(asset.getTitillium_regular());
		Paragraph b2_suCcP = new Paragraph().add(b2_suCcText).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text b2_numeroCcPostaleText = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 1).Codeline12Boll).setFont(asset.getTitillium_bold());
		...
		Text b2_autorizzazioneText = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 1).AutorizCcp).setFont(asset.getTitillium_regular());
		...
		// Intestato a
		Rectangle b2_intestatoARectangle = new Rectangle(178, 406 - 160, 39, 12);
		Canvas b2_intestatoACanvas = new Canvas(pdfCanvas, b2_intestatoARectangle);
		Text b2_intestatoAText = new Text(LeggoAsset.INTESTATO_A).setFont(asset.getTitillium_regular());
		Paragraph b2_intestatoAP = new Paragraph().add(b2_intestatoAText).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Rectangle b2_intestatarioCCPostaleRectangle = new Rectangle(220, 407 - 160, 258, 10);
		//fine LP PG200420 - Errori nel pdf dell'avviso
		Canvas b2_intestatarioCCPostaleCanvas = new Canvas(pdfCanvas, b2_intestatarioCCPostaleRectangle);
		Text b2_intestatarioCCPostaleText = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 1).Descon60Boll)
				.setFont(asset.getRoboto_bold());
		...
		// Destinatario
		Rectangle b2_destinatarioRectangle3 = new Rectangle(178, 389 - 160, 47, 12);
		Canvas b2_destinatarioCanvas3 = new Canvas(pdfCanvas, b2_destinatarioRectangle3);
		Text b2_destinatarioText = new Text("Destinatario").setFont(asset.getTitillium_regular());
		Paragraph b2_destinatarioP = new Paragraph().add(b2_destinatarioText).setFontColor(ColorConstants.BLACK)
				.setFontSize(8).setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Rectangle b2_nomeDestinatarioRectangle3 = new Rectangle(229, 366 - 160, 170 - 10 - 9 - 25, 32);
		//fine LP PG200420 - Errori nel pdf dell'avviso
		Canvas b2_nomeDestinatarioCanvas3 = new Canvas(pdfCanvas, b2_nomeDestinatarioRectangle3);
		Text b2_nomeDestinatarioText = new Text(documento.DatiAnagrafici.get(0).Denominazione1)
				.setFont(asset.getTitillium_bold());
		...
		// Oggetto Pagamento
		Rectangle b2_oggettoPagamentoRectangle3 = new Rectangle(178, 371 - 160, 68, 12);
		Canvas b2_oggettoPagamentoCanvas3 = new Canvas(pdfCanvas, b2_oggettoPagamentoRectangle3);
		Text b2_oggettoPagamentoText3 = new Text(LeggoAsset.OGGETTO_PAGAMENTO).setFont(asset.getTitillium_regular());
		Paragraph b2_oggettoPagamentoP3 = new Paragraph().add(b2_oggettoPagamentoText3)
				.setFontColor(ColorConstants.BLACK).setFontSize(8).setMargin(0)
		...
		// Oggetto Pagamento String
		Rectangle b2_oggettoPagamentoStringRectangle3 = new Rectangle(250, 350 - 160, 213, 30);
		Canvas b2_oggettoPagamentoStringCanvas3 = new Canvas(pdfCanvas, b2_oggettoPagamentoStringRectangle3);
...
// 		Data matrix
		...
		// Codice Avviso
		Rectangle b2_codiceAvvisoRectangle2 = new Rectangle(179, 348 - 160, 49, 12);
		...
		// Tipo
		Rectangle b2_tipoRectangle = new Rectangle(329, 348 - 160, 15, 12);
		Canvas b2_tipoCanvas = new Canvas(pdfCanvas, b2_tipoRectangle);
		Text b2_tipoText = new Text(LeggoAsset.TIPO).setFont(asset.getTitillium_regular());
		Paragraph b2_tipoP = new Paragraph().add(b2_tipoText).setFontColor(ColorConstants.BLACK).setFontSize(8)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text b2_p1Text = new Text(LeggoAsset.P1).setFont(asset.getTitillium_bold());
		Paragraph b2_p1P = new Paragraph().add(b2_p1Text).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		b2_p1Canvas.add(b2_p1P);
		b2_p1Canvas.close();

		// Cod. Fiscale Ente Creditore
		Rectangle b2_cfEnteCreditoreRectangle2 = new Rectangle(375, 348 - 160, 90, 12);
		...
		// Bollettino Postale PA
		Rectangle b3_bollettinoPostalePaRectangle = new Rectangle(29, 462 - 320, 115, 15);
		Canvas b3_bollettinoPostalePaCanvas = new Canvas(pdfCanvas, b3_bollettinoPostalePaRectangle);
		Text b3_bollettinoPostalePaText = new Text(LeggoAsset.BOLLETTINO_POSTALE_PA).setFont(asset.getTitillium_bold());
		...
		Paragraph b3_numeroRataP = new Paragraph().add(b3_numeroRataText1).add(b3_numeroRataText2)
				.add(b3_numeroRataText3).add(b3_numeroRataText4).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text b3_suCcText = new Text(LeggoAsset.SUL_CC).setFont(asset.getTitillium_regular());
		Paragraph b3_suCcP = new Paragraph().add(b3_suCcText).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text b3_numeroCcPostaleText = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 2).Codeline12Boll).setFont(asset.getTitillium_bold());
		...
		Text b3_autorizzazioneText = new Text(documento.DatiBollettino.get(bollettinoDiPartenza + 2).AutorizCcp).setFont(asset.getTitillium_regular());
		...
		// Intestato a
		Rectangle b3_intestatoARectangle = new Rectangle(178, 406 - 320, 39, 12);
		Canvas b3_intestatoACanvas = new Canvas(pdfCanvas, b3_intestatoARectangle);
		Text b3_intestatoAText = new Text(LeggoAsset.INTESTATO_A).setFont(asset.getTitillium_regular());
		...
		Rectangle b3_intestatarioCCPostaleRectangle = new Rectangle(220, 407 - 320, 258, 10);
		//fine LP PG200420 - Errori nel pdf dell'avviso
		...
		// Destinatario
		Rectangle b3_destinatarioRectangle3 = new Rectangle(178, 389 - 320, 47, 12);
		Canvas b3_destinatarioCanvas3 = new Canvas(pdfCanvas, b3_destinatarioRectangle3);
		Text b3_destinatarioText = new Text("Destinatario").setFont(asset.getTitillium_regular());
		...
		Rectangle b3_nomeDestinatarioRectangle3 = new Rectangle(229, 366 - 320, 170 - 10 - 9 - 25, 32);
		//fine LP PG200420 - Errori nel pdf dell'avviso
		...
		// Oggetto Pagamento
		Rectangle b3_oggettoPagamentoRectangle3 = new Rectangle(178, 371 - 320, 68, 12);
		Canvas b3_oggettoPagamentoCanvas3 = new Canvas(pdfCanvas, b3_oggettoPagamentoRectangle3);
		Text b3_oggettoPagamentoText3 = new Text(LeggoAsset.OGGETTO_PAGAMENTO).setFont(asset.getTitillium_regular());
		...
		// Oggetto Pagamento String
		Rectangle b3_oggettoPagamentoStringRectangle3 = new Rectangle(250, 350 - 320, 213, 30);
...
// 		Data matrix
		...
		// Codice Avviso
		Rectangle b3_codiceAvvisoRectangle2 = new Rectangle(179, 348 - 320, 49, 12);
		...
		// Tipo
		Rectangle b3_tipoRectangle = new Rectangle(329, 348 - 320, 15, 12);
		Canvas b3_tipoCanvas = new Canvas(pdfCanvas, b3_tipoRectangle);
		Text b3_tipoText = new Text(LeggoAsset.TIPO).setFont(asset.getTitillium_regular());
		Paragraph b3_tipoP = new Paragraph().add(b3_tipoText).setFontColor(ColorConstants.BLACK).setFontSize(8)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		...
		Text b3_p1Text = new Text(LeggoAsset.P1).setFont(asset.getTitillium_bold());
		Paragraph b3_p1P = new Paragraph().add(b3_p1Text).setFontColor(ColorConstants.BLACK).setFontSize(10)
				.setMargin(0).setVerticalAlignment(VerticalAlignment.MIDDLE);
		b3_p1Canvas.add(b3_p1P);
		b3_p1Canvas.close();

		// Cod. Fiscale Ente Creditore
		Rectangle b3_cfEnteCreditoreRectangle2 = new Rectangle(375, 348 - 320, 90, 12);
...
//	}
	...
	private static String componiIndirizzo(String indirizzo, String citta, String provincia) {
...
//		if(importoSenzaVirgola.length() > 3) importoSenzaVirgola = togliZeriIniziali(importoSenzaVirgola);
//		if(importoSenzaVirgola.length() < 2) importoSenzaVirgola = "00" + importoSenzaVirgola;
//		if(importoSenzaVirgola.length() < 3) importoSenzaVirgola = "0" + importoSenzaVirgola;
//		for(int cursore = importoSenzaVirgola.length() - 5; cursore >= 1; cursore -= 3) {
//			importoSenzaVirgola = importoSenzaVirgola.substring(0, cursore + 1 ) + "." +  importoSenzaVirgola.substring(cursore, importoSenzaVirgola.length() );
//		}
//		String importoConVirgola = new StringBuilder(importoSenzaVirgola).insert(importoSenzaVirgola.length() - 2, ",")
//				.toString();
		return importoConVirgola;
	}
	
//	private static String togliZeriIniziali(String importoConZeri) {
//		String importoSenzaZeri = "";
//		for(int i = 0; i<importoConZeri.length(); i++) {
//			char c = importoConZeri.charAt(i);
//			if (c != '0') {
//				importoSenzaZeri = importoConZeri.substring(i);
//				return importoSenzaZeri;
//			}
//		}
//		return importoSenzaZeri;
//	}
...
//	private static boolean seDiPoste(String iban) {
//		boolean ibanPostale = false;
//		if(iban != null && iban.length()>9) {
//			if(iban.substring(5, 10).contentEquals("07601")) {
//				ibanPostale = true;
//			}
//		}
//
